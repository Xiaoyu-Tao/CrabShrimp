...
- **[2026-03-27]** 🎉 虾兵蟹将 v0.4.0 正式发布！
- **[2026-03-26]** v0.3.0 发布。
...
<p align="center">
  <img src="./pipeline.png" alt="虾兵蟹将架构图" width="860"/>
</p>
...
### Optimizer Agent 优化引擎（v0.4）

每次任务结束后，自动分析 Trace 信号和演化 delta，识别表现不佳的角色，生成针对性 system prompt 改进建议：

| 机制 | 说明 |
|------|------|
| **触发条件** | agent 贡献分下降超过 0.05，或步骤 result == "failure" |
| **排除角色** | Critic / Verifier（本身职责是否定）不被优化 |
| **输出** | 简洁行为指令，追加到该角色下次执行的 system prompt |
| **存储** | `prompt_optimizations` 表，每个 `(role, category)` 槽最多 5 条 |
| **注入** | 创建 Agent 时自动注入最优建议（与 Skill 注入机制相同） |
| **启用** | `--optimize` 或 `CRABSHRIMP_OPTIMIZER=true`（默认关闭） |
...
| `prompt_optimizations` | v0.4 Optimizer 自动生成的 Prompt 改进建议 |
...
| `CRABSHRIMP_OPTIMIZER` | `false` | 是否启用 Optimizer Agent（自动改写低效 Prompt） |
| `CRABSHRIMP_DISPLAY` | `true` | 是否启用 Rich 实时终端面板 |
...
| v0.4 | ✅ 已发布 | Optimizer Agent：分析 Trace 信号，自动改写低效 Prompt |
